...
// RAII helper that always unlocks the account when the transaction scope ends.
struct Guard {
  explicit Guard(Account& account) : account_(&account) { account_->Lock(); }
...
void Transaction::SaveToDataBase(Account& from, Account& to, int sum) {
  ...
  std::cout << "Balance " << from.id() << " is " << from.GetBalance()
            << std::endl;
  std::cout << "Balance " << to.id() << " is " << to.GetBalance()
            << std::endl;
}
